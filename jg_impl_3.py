...
import numpy as np
import sys
import calculation_constants as cc
...
def get_standard_unit_value(value,unit):
	if unit == 'C':
		...
		return value*cc.STD_UNIT_CONV_DICT[unit]
...
def calculatedconstants():
	...
	#------------------------------------------------------------------------
	tau = 1 - Temp*cc.TEMP_CRITICAL**-1 # fraction
	PH2O = cc.PRES_CRITICAL*e**(cc.TEMP_CRITICAL*Temp**-1*(cc.A1*tau+cc.A2*tau**1.5+cc.A3*\
		tau**3+cc.A4*tau**3.5+cc.A5*tau**4+cc.A6*tau**7)) # kPa
	#------------------------------------------------------------------------
	# Name:		 PIO2
	#------------------------------------------------------------------------
	...
	#------------------------------------------------------------------------
	Wbl = (1-Hct)*cc.WC_PLASMA+Hct*cc.WC_RBC # fraction
	#------------------------------------------------------------------------
	...
	#------------------------------------------------------------------------
	alphaO2 = alpha_o2_func(Temp, cc.WC_PLASMA) # M/kPa
	alphaCO2 = alpha_co2_func(Temp, cc.WC_PLASMA) # M/kPa
	#------------------------------------------------------------------------
	#----------- AND FIX INPUT UNITS
	BE = BE/1000 # input is mEq/l
	trueVO2 = VO2 # user-defined VO2 is an aspiration...
	#------------------------------------------------------------------------
	PACO2 = trueVO2*RQ*cc.STD_PRES*Temp*cc.STD_TEMP**-1*VA**-1 # Alveolar ventilation equation. Simplifies to PACO2=0.863*VCO2/VA under normal conditions
...
def sn_o2_f1(pn_o2, ph, pn_co2, dpg, temp): # Equation B.3
	...
	try:
		pno2p50 = (pn_o2*p50_func(ph, pn_co2, dpg, temp)**-1)**(1+cc.N0)
...
def sn_o2_f2_null(sats, cn_o2, p50_sn_o2):
	'''returns 0 '''
	return Wbl*alphaO2*p50_sn_o2*(sats*(1-sats)**-1)**((1+cc.N0)**-1) + \
		(4*HbMol)*sats - (cn_o2*(cc.STD_R*cc.STD_TEMP*cc.STD_PRES**-1*1e2)**-1)
...
def cn_o2_f1(pn_o2, ph, pn_co2, dpg, temp):
	...
	-sn_o2_f1(pn_o2,ph,pn_co2,dpg,temp))**-1)**((1+cc.N0)**-1)*alphaO2\
	+4*HbMol*sn_o2_f1(pn_o2,ph,pn_co2,dpg,temp))*(cc.STD_R*cc.STD_TEMP*cc.STD_PRES**-1*1e2)
...
def pn_o2_f1(sn_o2, p50_sn_o2):
	...
	return p50_sn_o2*(sn_o2*(1-sn_o2)**-1)**((1+cc.N0)**-1) # kPa
...
def k_ratio_func(pn_co2, ph, temp, wpl):
	...
	return (cc.K_PRIME_2*CO2*(1+cc.K_2_PRIME_2*hrbc**-1)
	 	+(1+hrbc*cc.K_2_PRIME_5**-1))*(cc.K_PRIME_3*CO2*(1+cc.K_2_PRIME_3*hrbc**-1)
		+(1+hrbc*cc.K_2_PRIME_6**-1))**-1 # unitless
...
def cn_co2_bicarb(ph, pn_co2):
	...
	return ((1-Hct)*cc.WC_PLASMA+Hct*cc.WC_RBC*0.69)*hh_f1(pn_co2, ph) # M
def cn_co2_hb_bound(pn_co2, pn_o2, ph):
	'''Calculate Hb-CO2 from PARTIAL PRESSURE'''
	k_prime_4 = (alphaO2*pn_o2)**cc.N0*k_ratio_func(pn_co2,ph,Temp,cc.WC_PLASMA)*(p50_func(ph, pn_co2, DPG, Temp)*alphaO2)**-(1+cc.N0)
	s_hb_co2 = ( \
	cc.K_PRIME_2*alphaCO2*pn_co2*(1+cc.K_2_PRIME_2*(10**-(ph+log(0.69,10)))**-1) + \
	cc.K_PRIME_3*alphaCO2*pn_co2*(1+cc.K_2_PRIME_3*(10**-(ph+log(0.69,10)))**-1)*k_prime_4*alphaO2*pn_o2
	)*( \
	cc.K_PRIME_2*alphaCO2*pn_co2*(1+cc.K_2_PRIME_2*(10**-(ph+log(0.69,10)))**-1) + \
	(1+(10**-(ph+log(0.69,10)))*cc.K_2_PRIME_5**-1) + \
	k_prime_4*alphaO2*pn_o2*(cc.K_PRIME_3*alphaCO2*pn_co2*(1+cc.K_2_PRIME_3*(10**-(ph+log(0.69,10)))**-1) + \
	(1+(10**-(ph+log(0.69,10)))*cc.K_2_PRIME_6**-1)) \
	)**-1
...
def cn_co2_f1(ph, pn_co2, pn_o2):
	...
	return (cn_co2_hb_bound(pn_co2,pn_o2,ph)+cn_co2_bicarb(ph,pn_co2)\
		+cn_co2_dissolved(pn_co2))*(cc.STD_R*cc.STD_TEMP*cc.STD_PRES**-1*1e2) # ml CO2 per 100ml blood STP
...
def hh_f1(pn_co2, ph):
	...
	return (cc.K_1*alphaCO2*pn_co2)*(10**-ph)**-1 # M
#----------------------------------------------------------------------------
...
def van_slyke_f1(ph, base_excess, sn_o2):
	...
	return ((base_excess*1e3 - 0.2*Hb*1e-1*(1-sn_o2))*zeta**-1 - beta*(ph-7.4) + cc.STD_BICARB*1e3)*1e-3 # M
#----------------------------------------------------------------------------
...
def acidbase_f1_null(ph,base_excess, pn_co2, pn_o2):
	...
	zeta = 1-(0.0143*Hb*1e-1)
	beta = 9.5+1.63*Hb*1e-1
	return ((base_excess*1e3 - 0.2*Hb*1e-1*(1-sn_o2))*zeta**-1 - beta*(ph-7.4) + cc.STD_BICARB*1e3)*1e-3 - hh_f1(pn_co2, ph) # null
...
def populate_alv_gas_eqn():
	global AGE; AGE={}
	global PACO2, PAO2
	AGE['PACO2'] = VO2*RQ*cc.STD_PRES*Temp*cc.STD_TEMP**-1*VA**-1 # Alveolar ventilation equation
...
def dxdt(inputs, t, vq, dm_o2_ivp, vc_ivp, cv_o2, cv_co2):
	...
	if (cv_co2 - cc_co2) != (cc_o2 - cv_o2):
		...
		pa_co2 = vq**-1*(cc_o2-cv_o2)*1e-2*rq*cc.STD_PRES*Temp*cc.STD_TEMP**-1 # kPa
	...
	rate_o2 = k_prime_c*alphaO2*60*(1-sats)*4*HbMol*cc.STD_R*cc.STD_TEMP*cc.STD_PRES**-1 # ml(O2).ml(bld)**-1.kPa**-1.min**-1
...
def dxdt_organ(inputs, t, dm_o2_ivp, vol_b, c_in_o2, c_in_co2, q, compound, qd, c_di_o2, c_di_co2):
	...
	p_organ_o2 = (c_do_o2*10*1000**-1) * cc.STD_PRES 
	k = cc.STD_PRES*Temp/cc.STD_TEMP
	...
	# now do O2
	k_prime_c = 1.25284e5+3.6917e4*e**(3.8200*sats) # M**-1.sec**-1
	theta = k_prime_c*alphaO2*60*(1-sats)*4*HbMol*cc.STD_R*cc.STD_TEMP*cc.STD_PRES**-1 # ml(O2).ml(bld)**-1.kPa**-1.min**-1
...
def run_organ(dtype, preorgan_cn_o2, preorgan_cn_co2):
	if dtype=="lung":
		...
		post_organ_o2 = (c_do_o2*10*1000**-1) * cc.STD_PRES*Temp/cc.STD_TEMP 
		post_organ_co2 = (c_do_co2*10*1000**-1) * cc.STD_PRES*Temp/cc.STD_TEMP 
